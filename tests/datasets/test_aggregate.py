...
def test_validate_all_metadata_logs_mismatched_robot_type(caplog):
    base_features = {
    ...
    all_metadata = [
        SimpleNamespace(
            repo_id="/data/datasets/base",
            root="/data/datasets/base",
            fps=30,
            robot_type="unitree_g1",
            features=base_features,
        ),
        SimpleNamespace(
            repo_id="/data/datasets/mismatch",
            root="/data/datasets/mismatch",
            fps=30,
            robot_type="unitree_g1_8520",
            features=base_features,
        ),
    ]

    with caplog.at_level(logging.INFO):
        with pytest.raises(ValueError, match="Same robot_type is expected"):
            validate_all_metadata(all_metadata)

    logs = "\n".join(record.getMessage() for record in caplog.records)
    assert "metadata 基准" in logs
    assert "/data/datasets/base" in logs
